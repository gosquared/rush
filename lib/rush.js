...
var Rush = function(opts){
  ...
  this.results = LRU({
    max: o.max,
    maxAge: o.maxAge,
    dispose: this.clearTTL // clear any pending ttl on disposed keys
  });
...
p.get = function(key, fetch, cb) {
  ...
  // run the fetch
  fetch(function(err) {
    if (!q.length) return; // only proceed if queue hasn't expired
    var results = slice.call(arguments);
    results.set(key, results); // save all result args, including err

    // different ttl on error results
    if (err) self.ttl(key, self.opts.errTTL);

    // iterate callback queue with results + empty
    runCallbacks.apply(q, results);
    // make queue falsy
    locks.del(key);
  });
};
...
module.exports = function create(fetch, opts) {
